...
public class SortActivity extends ListActivity implements Runnable, AdapterView.OnItemClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ListAdapter adapter = new ArrayAdapter<String>(SortActivity.this,android.R.layout.simple_list_item_1,data);//页面，布局，数据。adapter负责匹配
        ...
        handler=new Handler() {
            ...
            public void handleMessage(@NonNull Message msg) {
                if (msg.what == 5) { //数据获取
                    ...
                    ListAdapter adapter = new ArrayAdapter<String>(SortActivity.this,android.R.layout.simple_list_item_1,list2);//引入adapter,作为沟通数据和列表的桥梁
    ...
    public void run() {
        ...
        try{
            ...
            for(int p=1;p<=10;p++) {//控制和修改页码处
                Log.i(TAG,"run="+p);
                doc = Jsoup.connect("https://bing.ioliu.cn/ranking/?p="+p).get();
                Log.i(TAG,"run="+doc);
